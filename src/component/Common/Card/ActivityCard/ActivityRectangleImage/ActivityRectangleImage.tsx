...
const ActivityRectangleImage: React.FC<IActivityRectangleImage> = () => {
    ...
    const changeHandler: React.Dispatch<React.SetStateAction<File[]>> = (newImageFiles) => {
        setImageFiles((prev) => {
            const resolvedFiles =
                typeof newImageFiles === "function"
                    ? (newImageFiles as (prevState: File[]) => File[])(prev)
                    : newImageFiles;
            saveChangeToBlog("activities", resolvedFiles, indexNumber, "rectangleImage");
        ...
        <section>
            {mode === "create" && (
                <section className="h-48 w-full min-w-0 overflow-hidden rounded-xl">
                    {imageFiles.length === 0 && (
                        <UploadImage setImageFiles={changeHandler} />
                    )}
                    {imageFiles.length > 0 && (
